...
export function SelectedStationMap({
  ...
  useEffect(() => {
    ...
    void fetch(`${API}/stations/dynamic-sources?${parameters}`, { cache: 'no-store', signal: controller.signal })
      ...
      .then(async (raw: unknown) => { const bundle = raw as { identity: { matched_name?: string; eva?: string; ril?: string; station_number?: string; osm_type?: string; osm_id?: number }; sources?: { netex?: { status?: string }; era_rinf?: { status?: string }; openstreetmap?: { status?: string }; stada?: { status?: string }; fasta?: { status?: string; facility_count?: number } } }; const value = bundle.identity; const fetchedAt = new Date(); setIdentity({ name: value.matched_name, eva: value.eva, ril: value.ril, stationNumber: value.station_number, osm: value.osm_type && value.osm_id ? `${value.osm_type}/${value.osm_id}` : undefined }); setDbSources({ netex: bundle.sources?.netex?.status, rinf: bundle.sources?.era_rinf?.status, osm: bundle.sources?.openstreetmap?.status, stada: bundle.sources?.stada?.status, fasta: bundle.sources?.fasta?.status, facilities: bundle.sources?.fasta?.facility_count }); setSourceUpdated((current) => ({ ...current, stada: fetchedAt, netex: fetchedAt, rinf: fetchedAt, fasta: fetchedAt })); if (value.ril) { const platformParameters = new URLSearchParams({ name: value.matched_name || station.name, ril: value.ril }); const response = await fetch(`${API}/stations/platform-data?${platformParameters}`, { cache: 'no-store', signal: controller.signal }); if (response.ok) { const data = await response.json() as { platforms: AuthoritativePlatform[]; status: { db_infrago?: string; era_rinf?: string } }; let platforms = data.platforms; if (platforms.some((item) => item.usable_length_m != null)) localStorage.setItem(`station-platform-reference:${station.id}`, JSON.stringify(platforms)); else { try { const cached = JSON.parse(localStorage.getItem(`station-platform-reference:${station.id}`) || '[]') as AuthoritativePlatform[]; if (cached.length) { platforms = data.platforms.map((item) => ({ ...cached.find((entry) => entry.track === item.track), ...item, usable_length_m: item.usable_length_m ?? cached.find((entry) => entry.track === item.track)?.usable_length_m, rinf_platform_id: item.rinf_platform_id ?? cached.find((entry) => entry.track === item.track)?.rinf_platform_id, rinf_track_id: item.rinf_track_id ?? cached.find((entry) => entry.track === item.track)?.rinf_track_id, mapping_method: item.mapping_method ?? 'cached_reference', mapping_confidence: item.mapping_confidence ?? 'confirmed', mapping_evidence: item.mapping_evidence?.length ? item.mapping_evidence : ['last_successful_station_reference'] })); data.status.era_rinf = 'cached'; } } catch {} } setAuthoritativePlatforms(platforms); setDbSources((current) => ({ ...current, rinf: data.status.era_rinf })); setSourceUpdated((current) => ({ ...current, rinf: new Date() })); } } })
  ...
  useEffect(() => {
    ...
    void import('leaflet').then(async (L) => {
      ...
      try {
        ...
        const data = (await response.json()) as { elements: OsmElement[]; cache_used?: boolean };
        if (disposed) return;
        if (data.elements.length) localStorage.setItem(`station-osm-reference:${station.id}`, JSON.stringify(data.elements));
        else { try { data.elements = JSON.parse(localStorage.getItem(`station-osm-reference:${station.id}`) || '[]') as OsmElement[]; if (data.elements.length) { data.cache_used = true; setDbSources((current) => ({ ...current, osm: 'cached' })); } } catch {} }
        ...
        edges.forEach((edge) => {
          L.polyline(edge.geometry.map((point) => [point.lat, point.lon] as [number, number]), { color: '#00a6c7', weight: 5, opacity: .9 })
            .addTo(instance!);
          L.polyline(edge.geometry.map((point) => [point.lat, point.lon] as [number, number]), { color: '#00a6c7', weight: 18, opacity: 0 })
            .bindTooltip(`Gleis ${escapeHtml(edge.track || 'ohne Nummer')}`, { sticky: true, direction: 'top', className: 'track-hover-tooltip' }).addTo(instance!);
  ...
  const sourceState = (status?: string) => status === undefined ? 'loading' : status === 'cached' ? 'cached' : ['active', 'available_for_enrichment'].includes(status) ? 'active' : status === 'not_found' ? 'not_found' : 'unavailable';
  const sourceEntries = [
    ...
    { key: 'osm', name: 'OpenStreetMap', quality: 'D', state: dbSources.osm === 'cached' ? 'cached' : osmBaseMapStatus },
  ...
  const sourceStatusLabel = (state: string) => state === 'active' ? 'Aktiv' : state === 'cached' ? 'Cache / Referenzstand' : state === 'loading' ? 'Wird geladen' : state === 'not_found' ? 'Keine Stationsdaten' : 'Nicht erreichbar';
  const sourceFreshness = (key: string, state: string) => sourceUpdated[key]
    ...
    : state === 'cached' ? 'Letzter erfolgreich gespeicherter Stationsstand' : state === 'loading' ? 'Aktualität wird ermittelt' : state === 'not_found' ? 'Quelle erreichbar, kein passender Datensatz' : 'Kein erfolgreicher Abruf in dieser Sitzung';
  ...
  return (
    <section className="selected-station-card">
      ...
      <section className="generic-feature-card">
        ...
        <div className="source-grid">{sourceEntries.map((source) => <div className="source-row" key={source.name}><span className={`source-indicator ${source.state === 'active' ? 'source-active' : source.state === 'cached' ? 'source-cached' : source.state === 'unavailable' ? 'source-error' : 'source-pending'}`}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{source.name}</p><p className="mt-1 text-xs text-muted-foreground">Qualitätsklasse {source.quality}</p><p className="source-freshness">{sourceFreshness(source.key, source.state)}</p></div><span className={source.state === 'active' ? 'source-state-active' : source.state === 'cached' ? 'source-state-cached' : source.state === 'unavailable' ? 'source-state-error' : 'source-state-pending'}>{sourceStatusLabel(source.state)}</span></div>)}</div>
      </section>
      <section className="generic-feature-card">
        ...
        <div className="generic-platform-overview">{platformRows.map(({ track, edge, data }, index) => { const check = lengthComparison(edge, data); return <div className="generic-platform-row" key={`overview-${track}`}><strong>B{index + 1}</strong><div><span>Bahnsteig Gleis {track}</span><div>{edge ? <button type="button" className="track-pill track-pill-button" onClick={() => focusPlatformLength(edge)} title={`Gleis ${track} im Luftbild anzeigen`}>Gleis {track}</button> : <span className="track-pill">Gleis {track}</span>}{data?.rinf_platform_id ? <small>RINF {data.rinf_platform_id}{data.rinf_platform_id !== track ? ` → Gleis ${track}` : ''}</small> : <small>RINF nicht zugeordnet</small>}{check ? <small>{Math.abs(check.delta).toFixed(1)} m Abweichung · {check.level === 'low' ? 'gering' : check.level === 'check' ? 'prüfen' : 'auffällig'}</small> : null}</div></div><i className={check ? `platform-overview-${check.level}` : 'platform-overview-missing'} aria-hidden="true"/></div>; })}</div>
      </section>
