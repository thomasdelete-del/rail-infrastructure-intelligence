...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10">
      ...
      <StationMap points={mapPoints} focusObjectKey={focusObjectKey} coordinateEdit={coordinateEdit} onSelect={setSelectedKey} onBeginCoordinateEdit={setCoordinateEdit} onCoordinateChange={handleCoordinateChange} onCancelEdit={() => setCoordinateEdit(null)}/>
...
function PlatformDataTable({ reference, inventory, coordinateDrafts, onFocus, onEdit }: { reference: ReferenceStation | null; inventory: Inventory | null; coordinateDrafts: CoordinateDrafts; onFocus: (objectKey: string) => void; onEdit: (edit: CoordinateEdit) => void }) {
  ...
  return <section className="mt-7 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
    ...
    <div className="platform-data-scroll"><table className="platform-data-table"><thead><tr><th>Gleis</th><th>Bahnsteighöhe</th><th>Baulänge OSM</th><th>OSM-Daten bestätigt</th><th>Nettobaulänge DB</th><th>Abweichung OSM–DB</th><th>Nutzlänge</th><th>Anfang Geokoordinaten</th><th>Ende Geokoordinaten</th></tr></thead><tbody>
      {reference?.platform_edges.map((edge) => { const deviation = comparison(edge); const confirmed = Boolean(osmConfirmed[edge.object_id]); const dbNeedsReview = confirmed && deviation?.level === 'high'; return <tr key={edge.object_id} className={deviation?.level === 'high' ? 'row-deviation-high' : undefined}><td><button type="button" className="track-pill track-focus" onClick={() => onFocus(edge.object_id)}>Gleis {edge.track}</button></td><td>{cell(osmValue(edge, 'platform_height'), 'OpenStreetMap')}</td><td>{osmGeometryCell(edge)}</td><td><button type="button" className={confirmed ? 'osm-confirm osm-confirmed' : 'osm-confirm'} aria-pressed={confirmed} onClick={() => toggleOsmConfirmation(edge.object_id)}><ShieldCheck size={15}/>{confirmed ? 'Bestätigt' : 'Bestätigen'}</button></td><td><div className={dbNeedsReview ? 'db-length-review' : undefined}>{cell(value(edge, 'net_construction_length'), 'DB InfraGO')}{dbNeedsReview ? <span>DB-Wert prüfen: bestätigte OSM-Länge weicht wesentlich ab</span> : null}</div></td><td>{deviation ? <button type="button" className={`deviation deviation-${deviation.level} deviation-button`} onClick={() => onFocus(edge.object_id)} title="Auf der Karte anzeigen"><strong>OSM {Math.abs(deviation.delta).toFixed(1)} m {deviation.delta >= 0 ? 'länger' : 'kürzer'}</strong><span>als DB · {deviation.percent.toFixed(1)}% · {deviation.level === 'low' ? 'gering' : deviation.level === 'check' ? 'prüfen' : 'auffällig'}</span></button> : <span className="data-missing">Nicht vergleichbar</span>}</td><td>{usableLengthCell(edge)}</td><td>{coordinateCell(edge, 'start')}</td><td>{coordinateCell(edge, 'end')}</td></tr>; })}
