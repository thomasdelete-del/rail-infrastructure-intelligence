...
export default function Home() {
  ...
  const beginCoordinateEdit = useCallback((edit: CoordinateEdit) => {
    const track = edit.title.replace(/^Gleis\s+/i, '');
    setCoordinateEdit(edit);
    setSelectedKey(edit.objectKey);
    setFocusObjectKey(edit.objectKey);
    setAerialReviewRequest({
      objectKey: edit.objectKey,
      coordinateType: edit.coordinateType,
      analysis: aerialResults[track],
      nonce: Date.now(),
    });
  }, [aerialResults]);

  if (selectedStation && !selectedStation.available) return <main className="min-h-screen bg-background text-foreground"><header className="border-b border-border bg-[#071b2b] text-white"><div className="mx-auto flex max-w-[1440px] items-center gap-3 px-5 py-4 lg:px-10"><span className="grid h-10 w-10 place-items-center rounded-md bg-[#f5a623] text-[#071b2b]"><TrainFront size={22}/></span><div><p className="text-sm font-semibold tracking-wide">DB INFRASTRUKTURDATEN</p><p className="text-xs text-slate-300">Deutschlandweite Bahnhofsauswahl</p></div></div></header><div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10"><GermanyStationMap onSelect={setSelectedStation}/><SelectedStationMap key={selectedStation.id} station={selectedStation} onBack={() => setSelectedStation(null)}/></div></main>;
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10">
      ...
      <StationMap points={mapPoints} focusObjectKey={focusObjectKey} coordinateEdit={coordinateEdit} aerialReviewRequest={aerialReviewRequest} aerialResults={aerialResults} onSelect={setSelectedKey} onNavigateEndpoint={(objectKey, analysis, coordinateType) => { setCoordinateEdit(null); setSelectedKey(objectKey); setFocusObjectKey(objectKey); setAerialReviewRequest({ objectKey, analysis, coordinateType, nonce: Date.now() }); }} onBeginCoordinateEdit={beginCoordinateEdit} onCoordinateChange={handleCoordinateChange} onCancelEdit={() => setCoordinateEdit(null)}/>
      <PlatformDataTable reference={reference} inventory={inventory} coordinateDrafts={coordinateDrafts} aerialResults={aerialResults} onResetAerialResults={() => setAerialResults({})} onAerialResult={(objectKey, result) => setAerialResults((current) => ({ ...current, [objectKey]: result }))} onFocus={focusPlatform} onAerialReview={(objectKey, analysis, coordinateType) => { setCoordinateEdit(null); focusPlatform(objectKey); setAerialReviewRequest({ objectKey, analysis, coordinateType, nonce: Date.now() }); }} onApplySuggestion={applyCoordinateSuggestion} onEdit={beginCoordinateEdit}/>
