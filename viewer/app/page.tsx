...
export default function Home() {
  ...
  const [aerialReviewRequest, setAerialReviewRequest] = useState<{ objectKey: string; nonce: number; analysis?: AerialAnalysis } | null>(null);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10">
      ...
      <PlatformDataTable reference={reference} inventory={inventory} coordinateDrafts={coordinateDrafts} onFocus={focusPlatform} onAerialReview={(objectKey, analysis) => { focusPlatform(objectKey); setAerialReviewRequest({ objectKey, analysis, nonce: Date.now() }); }} onApplySuggestion={applyCoordinateSuggestion} onEdit={(edit) => { setCoordinateEdit(edit); focusPlatform(edit.objectKey); }}/>
...
function PlatformDataTable({ reference, inventory, coordinateDrafts, onFocus, onAerialReview, onApplySuggestion, onEdit }: { reference: ReferenceStation | null; inventory: Inventory | null; coordinateDrafts: CoordinateDrafts; onFocus: (objectKey: string) => void; onAerialReview: (objectKey: string, analysis?: AerialAnalysis) => void; onApplySuggestion: (objectKey: string, start: Coordinate, end: Coordinate) => void; onEdit: (edit: CoordinateEdit) => void }) {
  ...
  return <section className="mt-7 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
    ...
    {platformCheckEnabled ? <div className="platform-check-results" aria-live="polite">
      {reference?.platform_edges.map((edge) => { const result = aerialResults[edge.track]; const loading = aerialLoading[edge.track]; const showResult = () => onAerialReview(edge.object_id, result); return <article key={edge.track} role="button" tabIndex={0} onClick={showResult} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') showResult(); }} className={`platform-check-result ${result ? `platform-check-result-${result.status}` : ''}`}><button type="button" className="platform-result-track" onClick={(event) => { event.stopPropagation(); showResult(); }}>Gleis {edge.track}</button>{loading ? <><strong>Endpunkte werden geprüft</strong><span>Jeweils ±35 m um Anfang und Ende …</span></> : result ? <><strong>{result.status === 'plausible' ? 'OSM-Endpunkte im Luftbild bestätigt' : result.status === 'check' ? 'Endpunkte prüfen' : result.status === 'high' ? 'Endpunkt deutlich abweichend' : result.candidate_start || result.candidate_end ? 'Ein Endpunkt bestätigt' : 'Endpunkte nicht eindeutig erkennbar'}</strong>{result.start_shift_m !== undefined ? <span>Anfang: {Math.abs(result.start_shift_m) <= 3 ? 'OSM bestätigt' : `${result.start_shift_m >= 0 ? '+' : ''}${result.start_shift_m.toFixed(1)} m`} · {Math.round((result.start_confidence ?? result.confidence) * 100)}%</span> : <span>Anfang: nicht eindeutig</span>}{result.end_shift_m !== undefined ? <span>Ende: {Math.abs(result.end_shift_m) <= 3 ? 'OSM bestätigt' : `${result.end_shift_m >= 0 ? '+' : ''}${result.end_shift_m.toFixed(1)} m`} · {Math.round((result.end_confidence ?? result.confidence) * 100)}%</span> : <span>Ende: nicht eindeutig</span>}<div className="platform-result-actions"><button type="button" onClick={(event) => { event.stopPropagation(); showResult(); }}>Endpunkte auf Karte zeigen</button>{result.candidate_start && result.candidate_end && result.status !== 'plausible' ? <button type="button" onClick={(event) => { event.stopPropagation(); onApplySuggestion(edge.object_id, result.candidate_start!, result.candidate_end!); }}>Endpunkte übernehmen</button> : null}</div></> : <><strong>Wartet</strong><span>Noch nicht geprüft</span></>}</article>; })}
    </div> : null}
