...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-10 lg:py-10">
      ...
      <StationMap points={mapPoints} focusObjectKey={focusObjectKey} coordinateEdit={coordinateEdit} aerialReviewRequest={aerialReviewRequest} onSelect={setSelectedKey} onBeginCoordinateEdit={setCoordinateEdit} onCoordinateChange={handleCoordinateChange} onCancelEdit={() => setCoordinateEdit(null)}/>
      ...
      <PlatformDataTable reference={reference} inventory={inventory} coordinateDrafts={coordinateDrafts} onFocus={focusPlatform} onAerialReview={(objectKey, analysis, coordinateType) => { focusPlatform(objectKey); setAerialReviewRequest({ objectKey, analysis, coordinateType, nonce: Date.now() }); }} onApplySuggestion={applyCoordinateSuggestion} onEdit={(edit) => { setCoordinateEdit(edit); focusPlatform(edit.objectKey); }}/>
...
function PlatformDataTable({ reference, inventory, coordinateDrafts, onFocus, onAerialReview, onApplySuggestion, onEdit }: { reference: ReferenceStation | null; inventory: Inventory | null; coordinateDrafts: CoordinateDrafts; onFocus: (objectKey: string) => void; onAerialReview: (objectKey: string, analysis?: AerialAnalysis, coordinateType?: 'start' | 'end') => void; onApplySuggestion: (objectKey: string, start: Coordinate, end: Coordinate) => void; onEdit: (edit: CoordinateEdit) => void }) {
  ...
  return <section className="mt-7 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
    ...
    <div className="platform-check-panel">
      <div><strong>Bahnsteiganfänge und -enden prüfen</strong><span>Nur die lokalen Bereiche um die OSM-Endpunkte werden mit dem amtlichen Luftbild verglichen.</span></div>
    ...
    {platformCheckEnabled ? <div className="platform-check-results" aria-live="polite">
      {reference?.platform_edges.map((edge) => { const result = aerialResults[edge.track]; const loading = aerialLoading[edge.track]; const startCoordinate = coordinateValue(edge, 'start'); const endCoordinate = coordinateValue(edge, 'end'); return <article key={edge.track} className={`platform-check-result ${result ? `platform-check-result-${result.status}` : ''}`}><button type="button" className="platform-result-track" onClick={() => onAerialReview(edge.object_id, result, 'start')}>Gleis {edge.track}</button>{loading ? <><strong>Beide Endpunkte werden geprüft</strong><span>Anfang und Ende jeweils ±35 m …</span></> : result ? <><strong>{result.status === 'plausible' ? 'OSM-Endpunkte im Luftbild bestätigt' : result.status === 'check' ? 'Endpunkte prüfen' : result.status === 'high' ? 'Endpunkt deutlich abweichend' : result.candidate_start || result.candidate_end ? 'Ein Endpunkt bestätigt' : 'Endpunkte nicht eindeutig erkennbar'}</strong><button type="button" className="endpoint-result-jump" onClick={() => onAerialReview(edge.object_id, result, 'start')}><span><b>Anfang</b>{startCoordinate ? <small>{startCoordinate.latitude.toFixed(7)}, {startCoordinate.longitude.toFixed(7)}</small> : <small>OSM-Koordinate fehlt</small>}</span><strong>{result.start_shift_m !== undefined ? `${Math.abs(result.start_shift_m) <= 3 ? 'OSM bestätigt' : `${result.start_shift_m >= 0 ? '+' : ''}${result.start_shift_m.toFixed(1)} m`} · ${Math.round((result.start_confidence ?? result.confidence) * 100)}%` : 'nicht eindeutig'}</strong></button><button type="button" className="endpoint-result-jump" onClick={() => onAerialReview(edge.object_id, result, 'end')}><span><b>Ende</b>{endCoordinate ? <small>{endCoordinate.latitude.toFixed(7)}, {endCoordinate.longitude.toFixed(7)}</small> : <small>OSM-Koordinate fehlt</small>}</span><strong>{result.end_shift_m !== undefined ? `${Math.abs(result.end_shift_m) <= 3 ? 'OSM bestätigt' : `${result.end_shift_m >= 0 ? '+' : ''}${result.end_shift_m.toFixed(1)} m`} · ${Math.round((result.end_confidence ?? result.confidence) * 100)}%` : 'nicht eindeutig'}</strong></button>{result.candidate_start && result.candidate_end && result.status !== 'plausible' ? <div className="platform-result-actions"><button type="button" onClick={() => onApplySuggestion(edge.object_id, result.candidate_start!, result.candidate_end!)}>Endpunkte übernehmen</button></div> : null}</> : <><strong>Wartet</strong><span>Noch nicht geprüft</span></>}</article>; })}
    </div> : null}
    ...
    <div className="platform-data-note"><AlertTriangle size={16}/><p>Der Nutzlängen-Hinweis erscheint ausschließlich, wenn die RINF-Nutzlänge weniger als 5,0 m unter der DB-Nettobaulänge liegt. Er ändert keine Quelle. Gleis 49 wird nicht als 1a übernommen.</p></div>
  </section>;
}
